...
func getCmd() *exec.Cmd {
	if _, err := os.Stat("/usr/bin/qemu-arm-static"); err == nil {
		return exec.Command("/usr/bin/qemu-arm-static", append([]string{"-execve", "-0", os.Args[0], "/bin/sh"}, os.Args[1:]...)...)
	}
	return exec.Command("/usr/bin/qemu-aarch64-static", append([]string{"-execve", "-0", os.Args[0], "/bin/sh"}, os.Args[1:]...)...)
}
